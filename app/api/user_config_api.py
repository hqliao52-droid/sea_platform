...
async def get_config(user_info: UserModel = Depends(get_current_user)):
    ...
    try:
        ...
        return Result.error(
            result_code=ResultCode.SYSTEM_ERROR, msg=f"数据格式化失败: {str(e)}"
        )


@router.put("/insert_config", response_model=Result[UserPushConfigResponseSchema])
async def insert_config(
    config: UserPushConfigSchema, user_info: UserModel = Depends(get_current_user)
):
    if not user_info:
        return Result.error(result_code=ResultCode.TOKEN_INVALID_ERROR, msg="未登录")
    ...
    try:
        ...
        return Result.error(
            result_code=ResultCode.SYSTEM_ERROR, msg=f"数据验证失败:  {error_msg}"
        )
    except Exception as e:
        return Result.error(
            result_code=ResultCode.SYSTEM_ERROR, msg=f"数据格式化失败: {str(e)}"
