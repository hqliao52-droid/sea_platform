...
async def spider_techcrunch():
    ...
    for t in techcrunch.entries:
        ...
        news_data = {
            ...
            "content": r,
            "authors": (
                ", ".join([x["name"] for x in t["authors"]]) if t.get("authors") else ""
            ),
            "keywords": (
                ", ".join([x["term"] for x in t["tags"]]) if t.get("tags") else ""
            ),
            "category": (
                ", ".join([x["term"] for x in t["tags"]]) if t.get("tags") else ""
            ),
...
def spider_rss(req_url):
    ...
    for r in rss_result.entries:
        ...
        if author_list != None:
            authors = (
                ", ".join([x["name"] for x in r["authors"]]) if r["authors"] else ""
            )
        else:
            authors = ""
