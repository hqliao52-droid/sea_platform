...
class ChatMessageOperator:
    ...
    async def insert_chat_message(self,chat_message:ChatMessage):
        db = db_session()
        try:
    ...
    async def get_chat_message_by_pre_id(self,pre_id:int) ->ChatMessage:
        db = db_session()
        try:
            chat_message = await self.chat_message_curd.get_chat_message_by_pre_id(db,pre_id)
            return chat_message
        except Exception as e:
            self.logger.error(e)
            return None
    ...
    async def get_chat_message_by_user_id(self,user_id:int) -> List[ChatMessage]:
        db = db_session()
        try:
            chat_message = await self.chat_message_curd.get_chat_message_by_user_id(db,user_id)
            return chat_message
        except Exception as e:
            self.logger.error(e)
            return None
    ...
    async def get_dialog_history(self,user_id,session_id,current_id) -> List[ChatMessage]:
        db = db_session()
        try:
            chat_message = await self.chat_message_curd.get_dialog_history(db,user_id,session_id,current_id)
